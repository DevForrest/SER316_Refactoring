...
public class TaskListImpl implements ITaskList {
    ...
    public long calcSubTaskEffort(ITask t) {
        ...
        if (hasSubTasks(t.getID())) {
            ...
            for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
            	...
            	totalEffort = totalEffort + calcSubTaskEffort(e);
            }
    ...
    public CalendarDate getSubTaskFirstStartDate(ITask t) {
        ...
        if (hasSubTasks(t.getID())) {
	        ...
	        for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
	        	ITask e = (ITask) iter.next();
	        	CalendarDate dd = getSubTaskFirstStartDate(e);
	        	if(dd.before(d)) {
	        	    d = dd;
	        	}
	        }
    ...
    public CalendarDate getSubTaskLastEndDate(ITask t) {
        ...
        if (hasSubTasks(t.getID())) {
	        ...
	        for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
	        	ITask e = (ITask) iter.next();
	        	CalendarDate dd = getSubTaskLastEndDate(e);
	        	if(dd.after(d)) {
	        	    d = dd;
	        	}
	        }
...
//        Util.debug("Task " + t.getText());
        ...
        if (hasSubTasks(t.getID())) {
            ...
            for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
            	ITask e = (ITask) iter.next();
            	long[] subTaskCompletion = calcSubTaskCompletion(e);
