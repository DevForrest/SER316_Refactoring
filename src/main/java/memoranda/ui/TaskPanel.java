...
//		CurrentProject.getTaskList().adjustParentTasks(newTask);
    ...
    void calcTask_actionPerformed(ActionEvent e) {
        ...
        if(dlg.calcEffortChB.isSelected()) {
            t.setEffort(tl.calcSubTaskEffort(t));
        }
        
        if(dlg.compactDatesChB.isSelected()) {
            t.setStartDate(tl.getSubTaskFirstStartDate(t));
            t.setEndDate(tl.getSubTaskLastEndDate(t));
        }
        
        if(dlg.calcCompletionChB.isSelected()) {
            long[] res = tl.calcSubTaskCompletion(t);
